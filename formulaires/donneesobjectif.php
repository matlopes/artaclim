 ...
 */
function formulaires_donneesobjectif_charger_dist(
	$id_indicateur,$url
) {
	$valeurs = array();
	$valeurs['url']=$url;
 ...
 * 
 */
function formulaires_donneesobjectif_verifier_dist(
	$id_indicateur,$url
) {

	$erreurs = array();
	
	if(!is_numeric(_request('objectif'))){
		$erreurs['objectif'] = _T('artaclim:erreur_non_numerique');
	}
	if(!is_numeric(_request('avance'))){
		$erreurs['avance'] = _T('artaclim:erreur_non_numerique');
	}
 ...
 *
 */
function formulaires_donneesobjectif_traiter_dist(
	$id_indicateur,$url
) {
	$res=array();
	...
	$valeurs['objectif'] = _request('objectif');
	$valeurs['avance'] =  _request('avance');
